...
namespace Dungeonizer {
	...
	public class Dungeonizer : MonoBehaviour {
		...
		public GameObject wallPrefab;
		public GameObject doorPrefab;
		public GameObject spawnbox, Bossbox;
				...
				for (var i = 0; i < room_count; i++) {
					...
					if (rooms.Count == 0) {
						...
						this.roomMarginTemp = Random.Range(0, this.roomMargin - 1);
							...
							room.w = Random.Range(min_size, max_size);
							if (i == maximumRoomCount - 1) room.w = 30;
							if (room.w % 2 == 0) room.w += 1;

							room.h = Random.Range(min_size, max_size);
							if (i == maximumRoomCount - 1) room.h = 30;
							if (room.h % 2 == 0) room.h += 1;
		...
		public void Generate()
		{
			...
			foreach(MapTile mapTile in Dungeon.map) {
				if (mapTile.type == 1 
					...
					{
						if (location.room.room_id != 10)
						{
                            location.inTheMiddle = true;
                            if (location.room.room_id == 29)
							{
                                GameObject spawnbox_ = Instantiate(Bossbox, new Vector3(roomCenterX, 0, roomCenterY), Quaternion.identity);
						...
						{
							//아이템을 생성시키는 로직
							GameObject itembox_ = Instantiate(item_box, new Vector3(roomCenterX, 0, roomCenterY), Quaternion.identity);
							itembox_.transform.parent = transform;

							itembox_.GetComponent<thisitem>()._item = ItemManager.Iteminstance.Radomitem();


							switch (itembox_.GetComponent<thisitem>()._item.itemtype)
							{
								case itemtype.일반: itembox_.transform.GetChild(0).gameObject.SetActive(true); break;
								case itemtype.레어: itembox_.transform.GetChild(1).gameObject.SetActive(true); break;
								case itemtype.유니크: itembox_.transform.GetChild(2).gameObject.SetActive(true); break;
								case itemtype.에픽: itembox_.transform.GetChild(3).gameObject.SetActive(true); break;
								case itemtype.레전드: itembox_.transform.GetChild(4).gameObject.SetActive(true); break;
							}
		...
		void Start () {
			...
			GameManager.Instance.min_roomSize = minRoomSize;
            GameManager.Instance.max_roomSize = maxRoomSize;
